...
// Bypass macOS 15+ obsolete attribute on CGDisplayCreateImageForRect
static CGImageRef compat_CGDisplayCreateImageForRect(CGDirectDisplayID display, CGRect rect) {
	static CGImageRef (*func)(CGDirectDisplayID, CGRect) = NULL;
	if (!func) {
		func = dlsym(RTLD_DEFAULT, "CGDisplayCreateImageForRect");
	}
	if (func) return func(display, rect);
	return NULL;
}
...
MMBitmapRef copyMMBitmapFromDisplayInRect(MMRectInt32 rect, int32_t display_id, int8_t isPid) {
	...
	CGImageRef image = compat_CGDisplayCreateImageForRect(displayID, CGRectMake(o.x, o.y, s.w, s.h));
